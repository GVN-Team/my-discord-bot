...
TOKEN_FILE = "paypay_token.txt"     # トークン保存用ファイル名

def get_paypay_client() -> PayPay:
    """トークンがあればトークンで初期化し、なければパスワードログインする関数"""
...
def relogin_and_save() -> PayPay:
    """新規ログインして最新トークンを保存する関数"""
    print("PayPay: トークンを新規発行・更新中...")
    client = PayPay(phone=PAYPAY_PHONE, password=PAYPAY_PASSWORD)
    new_token = client.access_token
    with open(TOKEN_FILE, "w", encoding="utf-8") as f:
        f.write(new_token)
    print("PayPay: 新しいアクセストークンを保存しました。")
    return client

def safe_paypay_receive(link: str):
    """トークン切れ時に自動再ログインして受取を再試行する関数"""
    ...
    try:
        ...
        print(f"PayPay受取失敗。再ログインを試みます: {e}")
        # 再ログインしてトークン更新後、再試行
        client = relogin_and_save()
        link_info = client.link_check(link)
        receive_result = client.link_receive(link)
        return link_info, receive_result
...
class PurchaseModal(discord.ui.Modal, title="商品購入"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("エラー: 個数は1以上の正の整数で入力してください。", ephemeral=True)
            return

        # 2. 商品情報の取得と必要金額の計算
        item = vending_machines[self.v_id]["items"].get(self.item_id)
        if not item:
            await interaction.followup.send("エラー: 商品が見つかりませんでした。", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.followup.send(
                f"❌ 受取処理に失敗しました。SMS認証が必要になっている可能性があります。\nエラー内容: {e}",
                ephemeral=True
            )
...
class PaymentSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_modal(
            PurchaseModal(v_id=self.v_id, item_id=self.item_id, payment_method=selected)
        )
...
class EditItemModal(discord.ui.Modal, title="商品内容変更"):
    def __init__(self, v_id: str, item_id: str, current_item: dict):
        ...
        self.item_type = discord.ui.TextInput(
            label="タイプ(有限か無限以外だったらエラー)*", default=current_item["type"], required=True
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if self.item_type.value not in ["有限", "無限"]:
            await interaction.response.send_message(
                "エラー: タイプは「有限」または「無限」で入力してください。", ephemeral=True
            )
            return
        ...
        try:
            ...
            await interaction.response.send_message(
                "エラー: マネー・マネーライトは数値で入力してください。", ephemeral=True
            )
            return
...
async def delete_vending_machine(interaction: discord.Interaction, vending_machine_id: str):
    ...
    async def delete_cb(inter: discord.Interaction):
        ...
        await inter.response.edit_message(
            content=f"自販機「{target_name}」を完全に削除しました。", embed=None, view=None
        )
...
async def place_vending_machine(
    ...
    for item_id, item in vm_data["items"].items():
        ...
        embed.add_field(name=f"{emoji_str}{item['name']}", value=field_value, inline=False)
    ...
    async def buy_cb(inter: discord.Interaction):
        if not vm_data["items"]:
            await inter.response.send_message("この自販機には商品がありません。", ephemeral=True)
            return
        buy_view = discord.ui.View()
        buy_view.add_item(PurchaseItemSelect(vending_machine_id))
        await inter.response.send_message("購入する商品を選択してください。", view=buy_view, ephemeral=True)

    async def stock_cb(inter: discord.Interaction):
        stock_info = []
        for i_id, i_data in vm_data["items"].items():
            stock_num = "無限" if i_data["type"] == "無限" else str(i_data.get("stock", 0))
