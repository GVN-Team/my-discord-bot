...
@bot.tree.command(name="rank", description="ユーザーのレベルや発言数を確認します")
...
@bot.tree.command(name="setup_ticket", description="お問い合わせパネルを設置します")
...
async def user_info(interaction: discord.Interaction, member: Optional[discord.Member] = None):
    ...
    embed.add_field(name="サーバー参加日時", value=target.joined_at.strftime("%Y/%m/%d %H:%M:%S") if target.joined_at else "不明", inline=False)
...
# 6. 高度なロール管理機能
# =========================================================
@bot.tree.command(name="addrole", description="メンバーにロールを付与します")
...
@bot.tree.command(name="removerole", description="メンバーからロールを剥奪します")
@app_commands.checks.has_permissions(manage_roles=True)
...
@bot.tree.command(name="roleall", description="全一般メンバーに一括でロールを付与します")
@app_commands.checks.has_permissions(administrator=True)
...
@bot.tree.command(name="roles", description="サーバー内の全ロール一覧を表示します")
async def list_roles(interaction: discord.Interaction):
    ...
    description_lines = [f"• {r.mention} — **{len(r.members)}** 名 (ID: `{r.id}`)" for r in roles[:20]]
...
# =========================================================
# 7. モデレーション機能 (Kick, Ban, Mute, Clear)
# =========================================================
@bot.tree.command(name="clear", description="メッセージを一括削除します")
...
@bot.tree.command(name="kick", description="指定メンバーをキックします")
...
@bot.tree.command(name="ban", description="指定ユーザーをBANします")
...
@bot.tree.command(name="timeout", description="指定メンバーをタイムアウト(Mute)します")
...
# 9. ヘルプ & 起動処理
# =========================================================
@bot.tree.command(name="help", description="Botの利用可能なコマンド一覧を表示します")
async def custom_help(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🤖 Bot スラッシュコマンドヘルプ一覧",
        description="利用可能なスラッシュコマンド（/）の一覧です。",
        color=discord.Color.blue(),
        timestamp=datetime.utcnow()
    )
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        ...
        print(f"コマンドエラー発生: {error}", file=sys.stderr)
